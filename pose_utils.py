...
def cropFaceImage(frame, data_dict, factor, _alexNetSize):
    ...
    # Assuming that we needn't save tmp images
    # Assuming that face image is only one
    # Assuming we get image streams from webcam, we needn't load images from directories
    '''
    input:
        frame: current video frame
        data_dict: face bbox information. (startX, startY, width, height)
